...
import { NewsroomFilter } from '@/components/IntelligenceHub/NewsroomFilter';
import { getNewsroomItems, formatDate } from '@/lib/sanity/queries';
import type { NewsroomItemType, SanityNewsroomItem } from '@/types/sanity';
...
export default async function NewsroomPage({ searchParams }: NewsroomPageProps) {
  ...
  const items = await getNewsroomItems(activeType as NewsroomItemType | 'all', 20);

  // Group by year (server-side)
  const itemsByYear = items.reduce<Record<string, SanityNewsroomItem[]>>((acc, item) => {
    ...
    <>
      ...
      <div className="min-h-screen bg-white">
        ...
        <section className="py-12">
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            <NewsroomFilter activeType={activeType} />
